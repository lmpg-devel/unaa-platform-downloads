# Veille Archi Marches v1.0.20

- Windows et Android interrogent directement TED, BOSA, BMA et Vlaams Bouwmeester à chaque ouverture. Un événement natif couvre aussi le retour au premier plan Android.
- Le bouton « Actualiser les marchés » lance la même collecte, sans compte GitHub ni créneau horaire.
- Les données précédentes restent consultables pendant la collecte et hors connexion. Une source en échec est signalée ; son ancienne copie ne devient jamais une actualisation réussie.
- L'horodatage « Dernière actualisation complète » avance uniquement après consultation réussie des quatre sources.
- Les imports GitHub de secours ne refusent plus une exécution arrivée en retard. Le HTML web conserve la consultation du flux public : la collecte native concerne Windows et Android.
- Vérifié : deux lancements Windows, deux lancements Android signé, mise à jour Android 1.0.19 → 1.0.20, repli hors réseau et 14 tests automatisés. Voir QA-1.0.20.md.
- Réserve de livraison : l'installateur NSIS est une production provisoire non vérifiée en installation ; utiliser le ZIP Windows portable testé. Les appareils physiques et le retour d'arrière-plan Android après correction restent non testés.

# Veille Archi Marches v1.0.19

## Vérification automatique du registre des communes

- Le registre Statbel des communes est vérifié automatiquement chaque dimanche.
- L'application affiche désormais la date de la dernière vérification, et non une ambiguë date de mise à jour.
- Les métadonnées distinguent la dernière vérification de la dernière modification réelle des fichiers Statbel.
- Les empreintes SHA-256 des fichiers français et néerlandais déterminent si les données ont effectivement changé.
- Une vérification sans changement conserve la date de modification réelle du registre.

# Veille Archi Marches v1.0.18

## Fiabilisation des mises à jour et des emails

- Les avis inconnus restent marqués non lus jusqu'à leur ouverture.
- Les ajouts, modifications et retraits sont calculés entre deux imports.
- Le flux public est refusé s'il devient vide, incohérent ou trop ancien.
- L'import manuel via GitHub est présenté comme une procédure explicite.
- L'envoi email peut utiliser Microsoft Graph et ses échecs obligatoires ne sont plus masqués.

# Veille Archi Marches v1.0.17

## Correctif mise a jour demandee v1.0.17

- Apres ouverture de l'import reel GitHub Actions, l'application surveille automatiquement le flux public pendant 10 minutes et affiche le nouvel import publie des qu'il arrive.
- Le tri `Nouveaux d'abord` garde un classement coherent: nouveaux avis non lus en tete, puis avis les plus recents.
- Quand aucun nouveau marche non lu n'existe, l'interface l'indique explicitement au lieu de laisser croire que le tri affiche des nouveautes.
- La relecture du flux public reste distincte d'un vrai import et continue d'utiliser les endpoints publics avec anti-cache.

# Veille Archi Marches v1.0.16

## Correctif publication flux v1.0.16

- `generatedAt` du flux public correspond maintenant au dernier import reel (`retrievedAt`) et ne change plus lors d'une simple release sans nouvel import.
- Le workflow `daily-opportunity-import.yml` refuse un import prive impossible a publier publiquement, et utilise le deploy key du depot public quand le token downloads est absent.
- L'echec SMTP du digest email ne masque plus le succes ou l'echec de publication du JSON public.
- L'application distingue `Dernier import reel publie` et `Dernier flux public charge`.
- La relecture du flux public ajoute un parametre anti-cache unique sur les endpoints GitHub Contents, GitHub Pages et GitHub Raw.

# Veille Archi Marches v1.0.15

## Actualisation donnees v1.0.15

- Separation explicite entre `Recharger flux public` et `Lancer nouvel import`.
- `Recharger flux public` relit uniquement le dernier `latest-opportunities.json` publie, sans declencher de scraping/import.
- `Lancer nouvel import` ouvre le workflow GitHub Actions officiel `daily-opportunity-import.yml`.
- Le message de retour indique maintenant l'heure du dernier import publie et precise lorsqu'aucun nouvel import n'a ete lance.

# Veille Archi Marches v1.0.14

Correction packaging Windows: `time` est epingle en `0.3.36` cote Tauri pour eviter l'incompatibilite transitive `cookie/time` apparue sur Rust stable.

## Packaging v1.0.14

- Alignement des versions npm, Tauri, Cargo, Android et manifeste UNAA sur `1.0.14`.
- Conservation des corrections v1.0.13 sur `Actualiser donnees` et imports resilients.

## Historique v1.0.13

Correction du bouton `Actualiser donnees`: l'application utilise d'abord l'API publique GitHub Contents pour lire le flux publie, puis garde `raw.githubusercontent.com` comme fallback.

## Packaging v1.0.13

- Alignement des versions npm, Tauri, Cargo, Android et manifeste UNAA sur `1.0.13`.
- Lecture prioritaire de `latest-opportunities.json` via l'API publique GitHub Contents pour eviter le cache raw.
- Import BMA et Vlaams Bouwmeester rendu resilient: une coupure TLS conserve le snapshot precedent au lieu de bloquer toute la publication quotidienne.

## Historique v1.0.12

Correction critique Android: l'application ne demande plus de jeton ou cle GitHub a l'utilisateur.

## Packaging v1.0.12

- Alignement des versions npm, Tauri, Cargo, Android et manifeste UNAA sur `1.0.12`.
- Remplacement du bouton `Relancer import` par `Actualiser donnees`.
- L'action d'actualisation recharge uniquement le flux public `latest-opportunities.json`.
- Suppression de la boite de dialogue demandant un jeton GitHub temporaire.
- Suppression des appels frontend a l'API GitHub Actions.
- Les marches restent mis a jour par le flux public; l'APK n'embarque aucun token, mot de passe ou chemin local.
...
Correction de la veille quotidienne et de ses diagnostics :

- les avis non lus restent marques `Nouveau` entre les imports et les redemarrages, jusqu a leur ouverture ;
- ajout de l action `Tout marquer comme lu` et migration non destructive de l ancien historique local ;
- affichage du bilan du dernier import : ajouts, modifications et retraits ;
- clarification du lancement manuel GitHub, avec confirmation explicite avant surveillance ;
- remplacement du libelle ambigu `Dernier registre` par `Registre communes mis a jour` ;
- prise en charge de Microsoft Graph OAuth pour le digest, avec repli SMTP configurable ;
- les erreurs d envoi email ne sont plus masquees dans GitHub Actions ;
- alignement des versions npm, Tauri, Cargo, Android et manifeste UNAA sur `1.0.18`.

Configuration Microsoft Graph requise pour la production : voir `docs/MICROSOFT_GRAPH_EMAIL.md`.
